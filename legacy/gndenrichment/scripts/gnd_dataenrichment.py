...
def add_creation_date(graph, uri):
    ...
    if (URIRef(uri), dcterms.created, None) not in graph:
        graph.add(
            (URIRef(uri), dcterms.created, Literal(datetime.now())))  # onnly add creatioon date if it doesnt exist yet
            ...
            for date in data['results']:  # process every record of the page
                ...
                if date['imprint']['work']['title']:
                    for actor in date['imprint']['work']['actor']:
                        ...
                        if 'standardized_identifier' in actor['person'] and actor['person'][
                            'standardized_identifier'] is not None:
                            if 'identifier' in actor['person']['standardized_identifier'] and \
                                    actor['person']['standardized_identifier']['identifier'] is not None:
                                sID = actor['person']['standardized_identifier']['identifier']
                                if 'authority' in actor['person']['standardized_identifier'] and \
                                        actor['person']['standardized_identifier'][
                                            'authority'].strip() == 'VIAF Identifier':
                                ...
                                elif 'authority' in actor['person']['standardized_identifier'] and \
                                        actor['person']['standardized_identifier'][
                                            'authority'].strip() == "Library of Congress":
                                    sID = sID.replace('LOC ', '')
                                    sID = f'https://id.loc.gov/authorities/names/{sID}'
                                    graph.add((URIRef(uri), owl.sameAs, (Literal(sID))))
                                else:
                                    print(name, 'on Page: ', b_page, 'has: ',
                                          actor['person']['standardized_identifier']['authority'], 'as identifier.')
                        role = actor['role']['name']
                        graph.add((URIRef(uri), jl.occupation,
                                   (Literal(role))))  # add role in context with book as occupation
