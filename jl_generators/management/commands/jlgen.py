...
class Command(BaseCommand):
    help = "Listet Generatoren, führt sie aus, lädt in Fuseki und prüft Zeitpläne."
    ...
    def add_arguments(self, parser):
        ...
        p_run.add_argument("--slug", help="einzelner Generator (Slug)")
        p_run.add_argument("--load", action="store_true", help="nach Generierung auch laden")
        ...
        sub.add_parser("check")  # cron: prüft DB-Konfig/Enable und triggert falls nötig
    ...
    def handle(self, *args, **opts):
        ...
        if cmd == "list":
            ...
            self.stdout.write(self.style.ERROR("Unbekannter Befehl"))
    ...
    def _loader_load(self, slug: str, ds_root: Path, ttl_path: str | None):
        ...
        # nutzt judaicalink-loader als Subprozess (oder direkte Lib-API, wenn gewünscht)
        fuseki_url = os.environ.get("JL_FUSEKI_URL")  # z.B. http://localhost:3030/judaicalink
        if not fuseki_url:
            self.stdout.write(self.style.WARNING("JL_FUSEKI_URL nicht gesetzt – skip load"))
            return
    ...
    def _load(self, slug: str | None, only_newer: bool=False):
        # Optional: prüfe pro Datensatz timestamp der zuletzt geladenen Graph-Version
        # und rufe _loader_load falls neueres TTL existiert.
        pass

    def _check(self):
        # Cron: simple Logik – starte alle enabled Generatoren,
        # in v2 kannst du BYDAY/BYHOUR etc. aus DB interpretieren.
