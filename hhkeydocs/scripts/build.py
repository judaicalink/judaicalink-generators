...
Source: https://schluesseldokumente.net/
"""
...
# ---------- Repo root for standalone import ----------
...
def sanitize_literal(text: Optional[str]) -> str:
    """
    Cleans a literal string by removing quotes and line breaks.
...
def clean_url_string(string: str) -> str:
    """
    Cleans a string to be URL-friendly by removing special characters
...
def compress_file(path: Path) -> Path:
    """
    Compresses the given file using gzip and returns the path to the compressed file.
    :param path: The path to the file to be compressed.
    :return: The path to the compressed .gz file.
    """
...
def copy_to_dumps(slug: str, files: list[Path]) -> list[Path]:
    """
    Copies the given files to the dumps directory for the given slug.
    JL_DUMPS_ROOT/<slug>/current/   (default: /data/dumps)
    :param slug: The dataset slug.
...
def get_viaf_id(gnd_id: str) -> Optional[str]:
    """
    Fetches the VIAF ID corresponding to the given GND ID using the lobid.org service.
    :param gnd_id: The GND identifier as a string.
    :return: The VIAF ID as a string if found, otherwise None.
    """
    try:
...
def fetch_person_ids(beacon_url: str) -> list[int]:
    """
    Fetches person IDs from the given beacon URL.
    :param beacon_url: The URL of the beacon to fetch IDs from.
    :return: A list of person IDs as integers.
    """
    ids: list[int] = []
...
def fetch_org_ids(beacon_url: str) -> list[int]:
    """
    Fetches organisation IDs from the given beacon URL.
    :param beacon_url: The URL of the beacon to fetch IDs from.
    :return: A list of organisation IDs as integers.
    """
    ids: list[int] = []
    res = requests.get(beacon_url, timeout=30)
    soup = BeautifulSoup(res.content, "html.parser")
...
def fetch_place_ids(index_url: str) -> list[str]:
    """
    Fetches place IDs from the given index URL.
    :param index_url: The URL of the index page to fetch place IDs from.
    :return: A list of place ID strings.
    """
    links: list[str] = []
...
# ---------- Genaterate Graph (Data) ----------
def build_hhkeydocs_graph(g: Graph) -> None:
    """
    Fetches place-, persons- and organisations data from schluesseldokumente.net
    and fills the graph (no serialization).
    :param g: The RDFLib Graph to populate.
    :return: None
    """
    # Prefixes
    g.bind("skos", SKOS)
    ...
    for gid in tqdm(person_ids, desc="Persons"):
        ...
        try:
            ...
            g.add((uri, DCTERMS.created, Literal(datetime.now().isoformat(), datatype=XSD.dateTime)))
...
def parse_args(argv: list[str] | None = None):
    ...
    p.add_argument("--only-newer", action="store_true",
                   help="Load only when file is unchanged (hash/mtime -> skip)")
    ...
    # Available in TOML frontmatter of hhkeydocs.md
    p.add_argument("--graph", default=None, help="Named graph URI; Standard from TOML: graph")
    return p.parse_args(argv)


def main(argv: list[str] | None = None):
    """
    Main function to build the HHKeyDocs RDF dataset, generate metadata,
    optionally load it into a Fuseki triplestore, and copy files to dumps.
    ...
    if not args.meta_only:
        ...
        res = gen.run()  # writes output/hhkeydocs.ttl
        print(res)
        if res.get("status") != "success":
            return  # no gzip/dumps/meta if error
    ...
    # 3) Optional: load in Fuseki (data graph + meta graph)
    if args.load and not args.meta_only:
        from generator.loader import load_to_fuseki  # type: ignore
        graph_uri = args.graph or meta_front.get("graph")  # e.g. http://data.judaicalink.org/data/gba (here: hhkeydocs)
